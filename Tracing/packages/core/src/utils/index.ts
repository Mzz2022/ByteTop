...
import { isRegExp, isArray, isFunction, isNumber } from './typeUtils'
import { isInit } from './environmentUtils'
...
export function getLocationHref(): string {
  return typeof document !== 'undefined' && document.location ? document.location.href : ''; // 简化返回语句
}
...
export function unKnowToArray<T>(target: T[] | T): T[] {
  return [].concat(target);  //使用.`.concat`简化转换
}
